...
// –учной ввод удалЄн: добавлени€ должны использовать фабрику.\n\n    if (name.length() >= 31) {
        throw exception("Name too long (max 30 chars)");
    }
    p.setName(name);

    cout << "    ¬ведите номер телефона (формат 062-xxx, 121<=xxx<=333): " << color(infoColor);
    string phone;
    getline(cin, phone);
    ...
    cout << "    ¬ведите тариф (стоимость за минуту): " << color(infoColor);
    double tariff;
    if (!(cin >> tariff)) {
        cin.clear();
    ...
    cout << "    ¬ведите скидку (0-100%): " << color(infoColor);
    int discount;
    if (!(cin >> discount)) {
        cin.clear();
        cin.ignore(cin.rdbuf()->in_avail(), '\n');
    ...
    cout << "    ¬ведите врем€ в минутах: " << color(infoColor);
    int timeMin;
    if (!(cin >> timeMin)) {
        cin.clear();
        cin.ignore(cin.rdbuf()->in_avail(), '\n');
    ...
    cout << "    ¬ведите дату (день мес€ц год): " << color(infoColor);
